...
class Validate{
	...
	public function showErrors(){
		...
		if(!empty($this->errors)){
			$xhtml .= '<dl id="system-message"><dt class="error"></dt><dd class="alert alert-danger"><ul>';
	...
	private function validatePassword($element, $options){		
		if($options['action'] == 'add' || ($options['action'] == 'edit' && $this->source[$element] )){
			...
			if(!preg_match($pattern, $this->source[$element])){
				$this->setError($element, 'Mật khẩu bao gồm kí tự in hoa, chữ thường, số và kí tự đặc biệt');
			};
		}
